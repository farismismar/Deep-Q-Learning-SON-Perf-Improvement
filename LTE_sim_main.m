...
%    global enable_fcfs_handling
	...
	serv_cell =  1; %13; % is the serving cell with pos = 0,0.
    % end Faris code
    ...
    while networkClock.current_TTI < LTE_config.simulation_time_tti
        ...
        % Faris code is here
         if (live_network_alarms)
             before = sum(alarm_register);
             % This is the environment step()
                % Network (Player A) plays an action at random
                p_fault = 0.8;
                normal = binornd(1,p_fault);

                if normal == 1
                    % do nothing: action__ ==0 is do nothing
                    %fprintf('(network status is unchanged)\n')
                    action__ = 0; % this variable contains the network played action-- not the DQN's action
                    %alarm_register(action__) = 0; % always
                else
                    prob_action = randi([1,action_size-1]);
                    if prob_action == 1
                        if (eNodeBs(serv_cell).max_power ~= 40)
                            eNodeBs(serv_cell).max_power = 37; % removed 3 dB loss of sig
                            fprintf('ALARM: Cell %d loss of signal 3 dB.\n', serv_cell)
                            action__ = 1;
                            alarm_register(action__) = alarm_register(action__) + 1;
                         else
                            fprintf('(network status is unchanged)\n')
                        end
                     elseif prob_action == 2
                        azimuths = [90, 210, 330];
                        corr_azimuth = azimuths(serv_cell);% - 12); % since we are doing cells 13, 14, 15.

                        theta = corr_azimuth + randsample([-30,30], 1); % changed azimuth by 30 deg due to wind or something
                        if (eNodeBs(serv_cell).azimuth == corr_azimuth)
                            eNodeBs(serv_cell).azimuth = theta;
                            fprintf('ALARM: Cell %d impacted by wind.  Azimuth has changed to %d.\n', serv_cell, theta)
                            action__ = 2;
                            alarm_register(action__) = alarm_register(action__) + 1;
                        else
                            fprintf('(network status is unchanged)\n')
                        end
                      elseif prob_action == 3
                        if (eNodeBs(serv_cell).electrical_downtilt == 4)
                            eNodeBs(serv_cell).electrical_downtilt = 8;
                            fprintf('ALARM: Cell %d impacted by wind.  Tilt has changed to 8 degrees.\n', serv_cell)
                            action__ = 3;
                            alarm_register(action__) = alarm_register(action__) + 1;
                        else
                            fprintf('(network status is unchanged)\n')
                        end 
                      elseif prob_action == 4
                        if (eNodeBs(serv_cell).max_power == 37)
                            eNodeBs(serv_cell).max_power = 40; % removed 3 dB loss of sig
                            fprintf('ALARM: Cell %d amplifier overpower alarm 3 dB.\n', serv_cell)
                            action__ = 4;
                            alarm_register(action__) = alarm_register(action__) + 1;
                         else
                            fprintf('(network status is unchanged)\n')
                        end
                      elseif prob_action == 5
                        if (eNodeBs(serv_cell).total_nTX ~= 1)
                            eNodeBs(serv_cell).total_nTX = 1; % the cell is not configured for transmit div
                            fprintf('ALARM: Cell %d transmit diversity disabled.\n', serv_cell)
                            action__ = 5;
                            alarm_register(action__) = alarm_register(action__) + 1;
                        else
                            fprintf('(network status is unchanged)\n')
                ...
                end % normal operation or alarm
                after = sum(alarm_register);
                if after >= before
                    next_state = 2;
                else
                    next_state = 1;
            ...
            if enable_intelligent_SON
                py.main.agent_remember_wrapper(state + zeros(1, state_size), action + zeros(1, action_size), reward, next_state + zeros(1, state_size), done);
                % DQN algorithm starts here.

                if py.main.agent_memory_length_diff_wrapper() > 0
                    ret_vals = py.main.agent_replay_wrapper();
                    ret_vals = cell(ret_vals);
                    Q_value = [Q_value, ret_vals{1}];
                    losses = [losses, ret_vals{2}.tolist];
                end

                state = next_state;

                % Find actions from the Q-Learning
                action = py.main.agent_act_wrapper(state);
                ...
                fix_action(eNodeBs, serv_cell, action);
                ...
                if (after < before)
                    ...
                    if (after > 0)
                        reward = -1; % if the alarm count was zero, no point in discounting reward.
                    else
                        reward = 0;
                    end
                end
              ...
              %  ret_value = py.main.env_step_wrapper( ones(1, 5) * double(action)); % action_size
                ...
                Actions = [Actions, action]; %#ok<*AGROW>
                Rewards = [Rewards, total_reward];
                Alarms = [Alarms, after];
            else
             % no SON here  
            %elseif enable_fcfs_handling == true
                % FCFS algorithm: Fix the fault that just happened in the previous TTI.
              %  fix_action(eNodeBs, serv_cell, action__);
           % else % this is the total randomness
                % Random algorithm: Fix actions at random only if they occurred
                %if (sum(alarm_register) > 0)
                if ~normal
                    rand_action = randi([1,action_size]); % datasample(find(alarm_register), 1); %
                    fix_action(eNodeBs, serv_cell, rand_action);
                end
             %   end
            end % algorith selection
        end    % Live alarms
