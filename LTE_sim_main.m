...
if isempty(UEs)
    ...
    alarm_count = 0;
    cell_down_register = zeros(1, length(eNodeBs)); % reset every episode.
    ...
    while networkClock.current_TTI < LTE_config.simulation_time_tti
        ...
        % Faris code is here
        % To do: collect data from network after the first TTI
        % N_cells = length(eNodeBs);
%         for u_ = 1:length(UEs)
%             %UEs(u_).print
%             %UEs(u_).pos
%             %UEs(u_).TB_SINR_dB
%             ue_id_ = floor(UEs(u_).id / Total_Time);
%             tti_ = (1 + mod(UEs(u_).id, Total_Time));
%             ue_pos_1_ = UEs(u_).pos(1);
%             ue_pos_2_ = UEs(u_).pos(2);
%             cqi_ = median(UEs(u_).eNodeB_signaling.TB_CQI);
%             rsrp_ = 10*log10(UEs(u_).rx_power_tb_in_current_tti); % in dBm
%             
%             network_data.x1 = [network_data.x1, ue_id_]; % gives the UE ID
%             network_data.x2 = [network_data.x2, tti_]; % gives the TTI            
%             network_data.x3 = [network_data.x3, ue_pos_1_]; % coords angle and distance?
%             network_data.x4 = [network_data.x4, ue_pos_2_]; 
%             network_data.x5 = [network_data.x5, cqi_]; % median CQI from nRX branches?
%            
%             if numel(rsrp_) == 0
%                 network_data.x7 = [network_data.x7, -Inf];
%             else
%                 network_data.x7 = [network_data.x7, rsrp_];
%             end
%             
%             try
%                 sinr_dB_ = mean(UEs(u_).link_quality_model_output.SINR_dB(:));
%                 network_data.x6 = [network_data.x6, sinr_dB_];    % in dB     
%             catch 
%                 warning('Problem obtaining SINR: setting it to invalid.');
%                 network_data.x6 = [network_data.x6, -Inf];    % in dB     
%             end
%         end       
%         
%         X = [network_data.x1(:), network_data.x2(:), network_data.x5(:), network_data.x6(:), network_data.x7(:)];
% 
%         % Save the data to be used by Python
%         header = {'UE_ID' 'TTI' 'Median_CQI' 'SINR_dB' 'RSRP_dBm'}; 
%         textHeader = strjoin(header, ',');
% 
%         fid = fopen('network_data.csv', 'w'); 
%         fprintf(fid,'%s\n', textHeader);
%         fclose(fid);
% 
%         dlmwrite('network_data.csv', X, '-append');      
        ...
        if (live_network_alarms)
            if mod(networkClock.current_TTI,2) == 1
                ...
                if prob_action <= 5
                    % do nothing, 5/9 of the time. (action__ = 1 is do
                    % nothing)
                ...
                elseif prob_action == 6 && cell_down_register(neigh_cell) ~= 1
                    if (eNodeBs(neigh_cell).max_power ~= -inf) %|| (eNodeBs(neigh_cell).always_on == 1)
                        ...
                        cell_down_register(neigh_cell) = 1;
                    else
                        fprintf('(network status is unchanged)\n')
                    end
                elseif prob_action == 7
                    if (eNodeBs(serv_cell).nTX ~= 1)
                        eNodeBs(serv_cell).nTX = 1; % the cell is not configured for transmit div
                        fprintf('ALARM: Serving cell transmit diversity disabled.\n')
                        action__ = 3;
                        alarm_register(action__) = 1;
                    else
                        fprintf('(network status is unchanged)\n')
                    end
                elseif prob_action == 8
                    if (eNodeBs(serv_cell).antenna.max_antenna_gain ~= 11)
                        eNodeBs(serv_cell).antenna.max_antenna_gain = 11; % remove some power (6 dB) as a result of issues (the antenna is 17 dBi gain)
                        fprintf('ALARM: Serving cell loss of signal 6 dB.\n')
                        action__ = 4;
                        alarm_register(action__) = 1;
                    else
                        fprintf('(network status is unchanged)\n')
                    end 
                elseif prob_action == 9
                    theta = randsample([120,180], 1); % changed azimuth by 30 deg due to wind or something
                    if (eNodeBs(serv_cell).azimuth == 150)
                        eNodeBs(serv_cell).azimuth = theta;
                        fprintf('ALARM: Serving cell impacted by wind.  Azimuth has changed to %d.\n', theta)
                        action__ = 5;
                        alarm_register(action__) = 1;
                    else
                        fprintf('(network status is unchanged)\n')
                ...
                % Now SON (Player B) plays a corrective action:
                if enable_intelligent_SON
                    % DQN algorithm:
                    % Find actions from the Q-Learning
                    action = py.main.agent_act_wrapper(state, reward);
                    
                    if isa(action,'py.numpy.int64')
                        action = 1 * action;
                    else
                        action = cast(action,'int64');
                    end
                        
                    ret_value = py.main.env_step_wrapper(action);
                    
                    % [next_state, ~, ~, ~]
                    next_state = ret_value(1);
                    next_state = next_state{1};
                    
                    % store these actions in an array
                    Actions = [Actions, action];
                    
                    % Perform the action `action' accordingly
                    cleared = fix_action(eNodeBs, serv_cell, neigh_cell, action);  % An option is to try neigh_cell = randsample(find(cell_down_register),1) in all algorithms
                    ...
                    elseif cleared
                        ...
                        reward = -1; % alarm count is unchanged
                    end

                    done = (sum(alarm_register) + sum(cell_down_register) == 0); % is defined when alarm count is zero.
                       
                    % Rewards are here and not in the environment
                    if done == true
                     %   if networkClock.current_TTI < 0.8 * LTE_config.simulation_time_tti % 40 out of 50 TTIs
                     %       aborted = true;
                     %       reward = R_min;
                     %   else
                            reward = R_max;
                    ...
                    py.main.agent_remember_wrapper(state, action, reward, next_state, done)

                    % make next_state the new current state for the next frame.
                    state = next_state;
                 % no SON here  
                elseif enable_efficient_handling == true
                    % Oracle algorithm: Fix the action that just happened in the previous TTI.
